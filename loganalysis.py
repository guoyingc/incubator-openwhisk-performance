...
action_invocation_tag = 'POST /api/v1/namespaces/whisk.system/actions/'
trans_dict = {}
actions_dict = {}
...
def get_trans_number(trans_id):
    start=trans_id.find('_')
    end=trans_id.find(']')
    number=trans_id[start+1:end]
    print number
    new = number.rjust(10,'0')
    print new
    return new
...
def log_analysis(filename_controller,filenames_invoker,outputfile):
    ...
    while line:
        ...
        if line.find(action_invocation_tag)!=-1:
            ...
            trans.trans_num = get_trans_number(trans.trans_id)
            print trans.trans_id
    ...
    while line:
        if line.find(blocking_activation)!=-1:
            ...
            trans_num = get_trans_number(trans_id)            

            activation_time = get_finish_time(line, 0)
    
            if trans_dict.has_key(trans_num):
